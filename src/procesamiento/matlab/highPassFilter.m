function [ out ] = highPassFilter( noisySignal, Fs, WP, WS )
    ...
    [b1,a1]=butter(ord1,Wn1,'high');       % Coef. función de transferencia del filtro
    [h1,frec1]=freqz(b1,a1,'whole',Fs);   % Respuesta en frecuencia del filtro
    figure('Name','Filtro pasa altos','NumberTitle','off'); 
    plot(abs(h1)); title('Filtro pasa altos'); xlabel('Frecuencia [Hz]'); ylabel('Modulo de la ganancia, |H(jw)|')
